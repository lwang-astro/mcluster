...
\begin{itemize}
\item The virial ratio, $Q = -E_{kin}/E_{pot}$, where $E_{kin}$ is the total kinetic energy of the single cluster stars and $E_{pot}$ their potential energy, can be set with the parameter \texttt{-Q}. For output in $N$-body units (\texttt{-u0}), the stellar velocities are always generated in virial equilibrium ($Q=0.5$) and the chosen value of \texttt{-Q} is passed to \textsc{Nbody4/6} through the \texttt{.PAR} file, which scales the velocities internally. For the ASCII table of stars (\texttt{-C3}) with astrophysical units (\texttt{-u1}), the velocities are scaled to the requested virial ratio \texttt{-Q}. 
